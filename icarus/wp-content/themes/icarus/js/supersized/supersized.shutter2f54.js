...
(function($){
		...
		----------------------------*/
	 	_init : function(){
    		...
    		if (api.options.autoplay){
				...
				if (api.options.progress_bar) $(vars.progress_bar).stop().animate({bottom : -$(window).height()}, 0 );	//  Place progress bar
				jQuery('#progress-back').css('display','none');
			}
			...
			$(vars.tray_button).toggle(function(){
				$("#fullscreen-thumbnails-wrap").stop().animate({bottom : -$(vars.thumb_tray).height(), avoidTransforms : true}, 300 );
				if ($(vars.tray_arrow).attr('src')) $(vars.tray_arrow).attr("src", api.options.image_path + "button-tray-up.png");
				return false;
			}, function() {
				$("#fullscreen-thumbnails-wrap").stop().animate({bottom : 35, avoidTransforms : true}, 300 );
	...
	*/		
		    ...
		    $("#tray-panelnext").click(function() {
		    	api.nextSlide();
		    });
		    
		    $("#tray-panelprev").click(function() {
		    	api.prevSlide();
		    });
			...
			*/
			...
			$(window).resize(function(){
				...
				if (api.options.progress_bar && !vars.in_animation){
					...
					$(vars.progress_bar).stop().animate({bottom : -$(window).height()}, 0 );
		...
		----------------------------*/
	 	playToggle : function(state){
	 		...
	 		if (state =='play'){
				...
				jQuery('#progress-back').fadeIn('slow');
	 		}else if (state == 'pause'){
	 			// If image, swap to play
	 			if ($(vars.play_button).attr('src')) $(vars.play_button).attr("src", api.options.image_path + "play.png");
        		if (api.options.progress_bar && vars.is_paused)$(vars.progress_bar).stop().animate({bottom : -$(window).height()}, 0 );

				jQuery('#progress-back').fadeOut('slow');
	 		}
	...
	jQuery('.slideshow_title')
		...
		progressBar : function(){
    		$(vars.progress_bar).stop().animate({bottom : -$(window).height()}, 0 ).animate({ bottom:0 }, api.options.slide_interval);
    	}

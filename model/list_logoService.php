...
{
   return list_logo_Get("SELECT list_logo.id, list_logo.name, list_logo.name_en, list_logo.name_cn, list_logo.img, list_logo.date, list_logo.link, list_logo.link_fanpage, list_logo.content, list_logo.content_en, list_logo.content_cn, list_logo.position FROM  list_logo ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function list_logo_insert($obj)
{
    return exe_query("insert into list_logo (name,name_en,name_cn,img,date,link,link_fanpage,content,content_en,content_cn,position) values ('$obj->name','$obj->name_en','$obj->name_cn','$obj->img','$obj->date','$obj->link','$obj->link_fanpage','$obj->content','$obj->content_en','$obj->content_cn','$obj->position')",'list_logo');
}
//
function list_logo_update($obj)
{
    return exe_query("update list_logo set name='$obj->name',name_en='$obj->name_en',name_cn='$obj->name_cn',img='$obj->img',date='$obj->date',link='$obj->link',link_fanpage='$obj->link_fanpage',content='$obj->content',content_en='$obj->content_en',content_cn='$obj->content_cn',position='$obj->position' where id=$obj->id",'list_logo');
}
//
